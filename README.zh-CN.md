...
私人备注仅出现在 Owner 编辑、后台预览与完整备份中。JSON 导出包含类型标签、材料、酒谱、配料明细和私人备注，请私下保管；不生成公开下载地址。

### JSON 批量导入与导出

以 Owner 身份进入 `/studio/bar`，点击 **“导出完整 JSON（含私人备注）”** 下载完整备份。在 **“JSON 批量导入”** 中选择该文件，查看数量与酒名预览，再点击 **“确认导入为草稿”**。失败时保留已选文件，方便重试；成功后显示新增与跳过数量。

导入与导出共用现有的 `formatVersion: 1` 格式，以前导出的文件可直接使用。顶层字段为 `formatVersion`、`exportedAt`、`categories`、`ingredients`、`recipes` 和 `recipeIngredients`。手工整理批量酒谱时可参考[完整 JSON 示例](./docs/examples/bar-import.v1.json)，为新记录更换示例 UUID，并同步修改关联引用。

| 导入行为 | 规则 |
| --- | --- |
| 新酒谱 | 保留酒谱 ID、正文、私人备注、用量及步骤和配料顺序；一律新增为草稿，原来已发布或归档的也不例外 |
| 已有酒谱 | 相同 ID 整份跳过，保留当前内容、状态和修订号；同名但不同 ID 会视为新酒谱 |
| 类型标签 | 按 ID 或完全相同的名称复用，缺少的自动新增，已有内容不修改 |
| 材料 | 按 ID 或固定 `code` 复用并映射引用；ID 与 code 冲突时整批失败；`ice`、`water` 复用现有系统材料 |
| 管理字段 | 使用当前 Owner 与新时间戳；新酒谱修订号从 1 开始，无发布时间，配料明细生成新 ID |
| 校验与失败 | 检查格式版本、唯一 ID、字段和文件内引用完整性；所有数据库写入一起提交或回滚 |
| 文件限制 | UTF-8 JSON 最大 5 MiB；每批最多 1,000 个类型、5,000 种材料、1,000 份酒谱，每份最多 80 项配料 |

导入后在 Studio 审阅草稿并逐份发布。导入功能不需要新增数据库迁移，但须已执行现有酒单迁移。可省略的管理字段与大文件拆分方式详见[格式说明](./docs/bar.md#json-批量导入与导出)。
...
酒单管理查询、每项写入、批量导入和完整导出均独立验证 Owner 会话。公开查询采用字段白名单，不包含私人备注、草稿，以及仅被草稿引用的材料与类型。
